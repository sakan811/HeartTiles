...
export class MockSocketServer {
  ...
  async start() {
    ...
    this.io.use(async (socket, next) => {
      try {
        ...
        if (token.id && token.jti && token.email) {
          ...
          console.log(
            "Mock server: Authentication successful for user:",
            socket.data.userId,
          );
          return next();
        }
    ...
    this.io.on("connection", (socket) => {
      console.log(
        `Mock server: Client connected: ${socket.id} (User: ${socket.data.userId})`,
      );
  ...
  async handleJoinRoom(socket, data) {
    try {
      ...
      const baseUserId =
        socket.data.userId.split("-").slice(0, -2).join("-") ||
        socket.data.userId;

      // Check if player already in room (using base userId)
      const existingPlayer = room.players.find((p) => {
        const playerBaseId =
          p.userId.split("-").slice(0, -2).join("-") || p.userId;
      ...
      // Log for debugging
      console.log(
        `MockSocketServer: User ${socket.data.userName || socket.data.userId || "undefined"} joined room ${roomCode}`,
      );
  ...
  async startGame(room) {
    try {
      ...
      const { createInitialHand } = await import(
        "../factories/card-factories.js"
      );
  ...
  async handlePlaceHeart(socket, data) {
    try {
      ...
      const heartCard = playerHand.find(
        (card) => card.id === heartId && card.type === "heart",
      );
  ...
  async handleDrawHeart(socket, data) {
    try {
      ...
      const { generateRandomHeartCard } = await import(
        "../factories/card-factories.js"
      );
  ...
  async handleDrawMagicCard(socket, data) {
    try {
      ...
      const { generateRandomMagicCard } = await import(
        "../factories/card-factories.js"
      );
  ...
  async handleUseMagicCard(socket, data) {
    try {
      ...
      const { WindCard, RecycleCard, ShieldCard } = await import(
        "../../src/lib/cards.js"
      );
      ...
      if (card.type === "wind") {
        ...
        if (
          !tile ||
          !tile.placedHeart ||
          tile.placedHeart.placedBy === socket.data.userId
        ) {
        ...
        const opponent = room.players.find(
          (p) => p.userId === actionResult.removedHeart.placedBy,
        );
        if (opponent) {
          opponent.score = Math.max(
            0,
            opponent.score - actionResult.removedHeart.value,
          );
        }
  ...
  async handleEndTurn(socket, data) {
    try {
      ...
      const playerActions = room.gameState.playerActions[
        socket.data.userId
      ] || {
        drawnHeart: false,
        drawnMagic: false,
  ...
  handleDisconnect(socket) {
    ...
    if (socketInfo && socketInfo.roomCode) {
      ...
      if (room) {
        ...
        const baseUserId =
          socketInfo.userId.split("-").slice(0, -2).join("-") ||
          socketInfo.userId;
        ...
        if (room.players.length === 0) {
          ...
          console.log(
            `Mock server: Cleaned up empty room ${socketInfo.roomCode}`,
          );
        }
      }
    }
