...
export function ensureTestEnvironment() {
  if (
    ...
    console.warn(
      `Warning: NODE_ENV is "${process.env.NODE_ENV}", should be "test" or "integration" for testing`,
    );
  }
  return (
...
export async function connectToDatabase() {
  ...
  console.log(
    `Connecting to test MongoDB in ${process.env.NODE_ENV} environment`,
  );
  ...
  try {
    ...
    console.log(
      `MongoDB readyState: ${readyState} (0=disconnected, 1=connected, 2=connecting, 3=disconnecting)`,
    );
    ...
    if (readyState === 2) {
      ...
      await new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
          reject(
            new Error("Connection timeout during existing connection attempt"),
          );
        }, 10000);
...
export async function disconnectDatabase() {
  ...
  try {
    const readyState = mongoose.connection.readyState;
    console.log(
      `Disconnecting from MongoDB (readyState: ${readyState}) in ${process.env.NODE_ENV} environment`,
    );
    ...
    if (readyState !== 0) {
      ...
      try {
        ...
        console.log(
          `Disconnected from test MongoDB in ${process.env.NODE_ENV} environment`,
        );
      } catch (e) {
  ...
  try {
    ...
    for (const collectionName of collections) {
      ...
      while (retryCount < maxRetries && !cleared) {
        try {
          console.log(
            `Dropping collection: ${collectionName} (attempt ${retryCount + 1})`,
          );
          ...
          retryCount++;
          console.warn(
            `Drop attempt ${retryCount} failed for ${collectionName}:`,
            err.message,
          );
          ...
          if (retryCount >= maxRetries) {
            console.warn(
              `All drop attempts failed for ${collectionName}, trying fallback deleteMany`,
            );
            ...
            try {
              const Model =
                collectionName === "rooms"
                  ? Room
                  : collectionName === "playersessions"
                    ? PlayerSession
                    : User;
              const result = await Model.deleteMany({}).maxTimeMS(5000);
              console.log(
                `Fallback deleteMany for ${collectionName}: deleted ${result.deletedCount} documents`,
              );
              cleared = true;
              clearedCount++;
            } catch (deleteErr) {
              console.error(
                `Fallback deleteMany for ${collectionName} also failed:`,
                deleteErr.message,
              );
            }
            break;
    ...
    if (clearedCount === collections.length) {
      ...
      for (const collectionName of collections) {
        try {
          ...
          if (count > 0) {
            console.warn(
              `Collection ${collectionName} still has ${count} documents after clearing`,
            );
          ...
          if (err.code !== 26) {
            ...
            console.warn(
              `Could not verify collection ${collectionName}:`,
              err.message,
            );
          }
        }
    ...
    console.log(
      `Test database cleared in ${process.env.NODE_ENV} environment (${clearedCount}/${collections.length} collections cleared)`,
    );
  } catch (err) {
...
export function createTestUser(userData = {}) {
  return {
    _id:
      userData._id ||
      "test-user-" + Math.random().toString(36).substring(2, 11),
...
export function createTestPlayer(playerData = {}) {
  return {
    userId:
      playerData.userId ||
      "player-" + Math.random().toString(36).substring(2, 11),
