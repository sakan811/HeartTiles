...
import { connectToDatabase, disconnectDatabase } from './utils/server-test-utils.js';
...
export async function setup() {
  if (!isConnected) {
    try {
      console.log('Global setup: Connecting to database for integration tests...');
...
export async function teardown() {
  if (isConnected) {
    try {
      ...
      console.error('Global teardown: Database disconnection failed:', error.message);
    }
  }
}
