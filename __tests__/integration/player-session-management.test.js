...
describe("Player Session Management Integration Tests", () => {
  ...
  beforeAll(async () => {
    try {
      ...
      console.warn(
        "Database connection failed for player session tests:",
        error.message,
      );
    }
  });
  ...
  afterAll(async () => {
    try {
      ...
      console.warn(
        "Database disconnection failed for player session tests:",
        error.message,
      );
    }
  });
  ...
  beforeEach(async () => {
    ...
    try {
      ...
      console.warn(
        "Database clear failed for player session tests:",
        error.message,
      );
    }
  });
  ...
  describe("Player Data Migration", () => {
    ...
    it("should add new player when migrating non-existent player", async () => {
      const room = {
        players: [{ userId: "existingUser", name: "ExistingUser", score: 10 }],
        gameState: {
          playerHands: {
  ...
  describe("Session Persistence and Loading", () => {
    it("should load active player sessions from database", async () => {
      ...
      const mockLoadPlayerSessions = vi
        .spyOn(serverUtils, "loadPlayerSessions")
    ...
    it("should handle session loading errors gracefully", async () => {
      ...
      try {
        ...
        await expect(serverUtils.loadPlayerSessions()).rejects.toThrow(
          "Database error",
        );
      } finally {
        mockLoadPlayerSessions.mockRestore();
  ...
  describe("Reconnection Scenarios", () => {
          ...
          tiles: [
            {
              id: 0,
              color: "red",
              placedHeart: { placedBy: userId, value: 4 },
            },
    ...
    it("should handle reconnection when player has active shield", async () => {
      ...
      const room = {
        code: roomCode,
        players: [{ userId: userId, name: "TestUser", score: 15 }],
        gameState: {
          gameStarted: true,
  ...
  describe("Session Cleanup and Management", () => {
    ...
    it("should delete room when last player disconnects", async () => {
      ...
      const room = {
        code: roomCode,
        players: [{ userId: userId, name: "TestUser" }],
      };
