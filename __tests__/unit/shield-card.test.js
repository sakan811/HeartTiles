...
describe("Shield Card Functionality", () => {
      ...
      tiles: [
        {
          ...
          color: "red",
          emoji: "🟥",
          placedHeart: { color: "red", value: 2, placedBy: player1Id },
        },
        {
          id: 2,
          color: "yellow",
          emoji: "🟨",
          placedHeart: { color: "yellow", value: 1, placedBy: player2Id },
        },
  ...
  describe("Shield Activation", () => {
    it("should activate shield for player", () => {
      ...
      expect(mockGameState.shields[player1Id].protectedPlayerId).toBe(
        player1Id,
      );
    });
  ...
  describe("Shield Utility Methods", () => {
    it("should correctly check if player is protected", () => {
      expect(ShieldCard.isPlayerProtected(mockGameState, player1Id, 1)).toBe(
        false,
      );
    ...
    it("should correctly check if tile is protected", () => {
      ...
      expect(ShieldCard.isTileProtected(mockGameState, protectedTile, 1)).toBe(
        false,
      );
      expect(ShieldCard.isTileProtected(mockGameState, opponentTile, 1)).toBe(
        false,
      );
      expect(ShieldCard.isTileProtected(mockGameState, emptyTile, 1)).toBe(
        false,
      );
    ...
    it("should correctly check activation conditions", () => {
      ...
      expect(check.reason).toContain(
        "Cannot activate Shield while opponent has active Shield",
      );
    });
  });
  ...
  describe("Shield Card Integration", () => {
    ...
    it("should handle edge cases gracefully", () => {
      ...
      expect(ShieldCard.isTileProtected({}, mockGameState.tiles[0], 1)).toBe(
        false,
      );
    });
  });
