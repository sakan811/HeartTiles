...
describe('GameRoomPage Component (Lines 1-921)', () => {
        ...
        if (turnChangedHandler) {
          act(() => {
            turnChangedHandler({
              ...
              players: [{ userId: 'user1', name: 'Test User' }],
              shields: {
                user1: { active: true, remainingTurns: 1, activatedAt: Date.now(), activatedBy: 'user1' }
              }
            })
          })
      ...
      if (gameStartHandler) {
        act(() => {
          gameStartHandler({
            ...
            turnCount: 1,
            shields: {
              user2: { active: true, remainingTurns: 2, activatedAt: Date.now(), activatedBy: 'user2' }
            }
          })
        })
  ...
  describe('Magic Card Edge Cases and Invalid Targets', () => {
        ...
        act(() => {
          gameStartHandler({
            ...
            tiles: [
              ...
              { id: 2, color: 'blue', emoji: '🟦', placedHeart: { placedBy: 'user1' } } // Own heart
            ],
            playerHands: {
              user1: [
                { id: 'm1', type: 'wind', emoji: '💨', name: 'Wind Card', description: 'Blow away hearts' }
      ...
      if (gameStartHandler) {
        act(() => {
          gameStartHandler({
            ...
            playerHands: {
              user1: [
                { id: 'm1', type: 'recycle', emoji: '♻️', name: 'Recycle Card', description: 'Make tiles white' }
              ]
            },
            deck: { emoji: '💌', cards: 10 },
            magicDeck: { emoji: '🔮', cards: 10 },
    ...
    it('should handle magic card execution with shield protection', async () => {
      ...
      if (gameStartHandler) {
        ...
        expect(screen.getByText(/Shield active.*2 turns left/)).toBeInTheDocument()
      }
    })
  })
      ...
      if (gameStartHandler) {
        act(() => {
          gameStartHandler({
            ...
            turnCount: 1,
            playerActions: {
              user1: { drawnHeart: true, drawnMagic: true, heartsPlaced: 2, magicCardsUsed: 0 }
            }
          })
        })
        ...
        expect(screen.getByText('Hearts placed this turn: 2/2')).toBeInTheDocument()
      }
    })
      ...
      if (gameStartHandler) {
        act(() => {
          gameStartHandler({
            ...
            playerId: 'user1',
            tiles: [
              { id: 1, color: 'red', emoji: '🟥', placedHeart: { placedBy: 'user2' } }
            ],
            playerHands: {
              user1: [
                { id: 'm1', type: 'wind', emoji: '💨', name: 'Wind Card', description: 'Blow away hearts' },
                { id: 'm2', type: 'recycle', emoji: '♻️', name: 'Recycle Card', description: 'Make tiles white' }
              ]
            },
            deck: { emoji: '💌', cards: 10 },
            magicDeck: { emoji: '🔮', cards: 10 },
            turnCount: 1,
            playerActions: {
              user1: { drawnHeart: true, drawnMagic: true, heartsPlaced: 0, magicCardsUsed: 1 }
            }
          })
        })
        ...
        expect(screen.getByText('Magic cards used this turn: 1/1')).toBeInTheDocument()
      }
    })
  ...
  describe('Connection State Changes and Error Handling', () => {
    it('should handle socket disconnection gracefully', async () => {
      ...
      const connectionIndicator = screen.getByText('Connecting...').previousElementSibling
    ...
    it('should handle room error for non-full room scenarios', async () => {
      ...
      if (roomErrorHandler) {
        ...
        expect(consoleSpy).toHaveBeenCalledWith('Game page: Room error:', 'Invalid room code')
        expect(mockRouter.push).not.toHaveBeenCalled()
    ...
    it('should handle heart-placed event with player actions update', async () => {
      ...
      if (heartPlacedHandler) {
        act(() => {
          heartPlacedHandler({
            tiles: [
              { id: 1, color: 'red', emoji: '🟥', placedHeart: { placedBy: 'user1', value: 2, score: 4 } }
            ],
      ...
      if (magicCardUsedHandler) {
        act(() => {
          magicCardUsedHandler({
            card: { id: 'm1', type: 'shield', emoji: '🛡️', name: 'Shield Card' },
            actionResult: {
              type: 'shield',
      ...
      if (gameStartHandler) {
        act(() => {
          gameStartHandler({
            ...
            playerHands: {
              user1: [
                { id: 'unknown1', emoji: '❓', name: 'Unknown Card', description: 'Mystery' }
              ]
            },
            deck: { emoji: '💌', cards: 10 },
            magicDeck: { emoji: '🔮', cards: 10 },
        ...
        expect(consoleSpy).toHaveBeenCalledWith('Unknown card type:', expect.any(Object))
        consoleSpy.mockRestore()
  ...
  describe('Component Lifecycle and Memory Management', () => {
    it('should handle missing playerId in game-start event', async () => {
      ...
      if (gameStartHandler) {
        ...
        expect(consoleSpy).toHaveBeenCalledWith('PlayerId not provided by server')
        consoleSpy.mockRestore()
      }
    })
  })
